...
rm -f $archive_file
cd ../../plt
tar -cvzf ../ctrl/account/$archive_file account
cd -
...
./run_remote.sh $ip $ssh_port $user $passwd "cd $install_dir -rf; find ./account -type d -print  | sed '1d' | grep -v 'seaslog' | xargs -i sudo chown bitnami:www-data -R {}"
./run_remote.sh $ip $ssh_port $user $passwd "cd $install_dir -rf; find ./account -type f -print  | grep -v 'seaslog' | xargs -i sudo chown bitnami:www-data -R {}"
